...
enum color{
	...
	LOWCYAN, GREY, BLUE, GREEN, CYAN, RED, PURPLE, YELLOW, WHITE
}; // Перечисление цветов
const int width = 80;// Ширина игрового поля
const int height = 40;// Высота игрового поля
int field[height][width];// Игровое поле
const int startLengthSnake = 3;// Начальная длина змеи
const int startSpeed = 120;// Начальная скорость движения
...
COORD *delete2; // Деление на 2
HANDLE h = GetStdHandle(STD_OUTPUT_HANDLE);
using namespace std;
Snake_EX::Snake_EX()
{
}
void Snake_EX::SetCur(int x, int y)
{
	HANDLE h = GetStdHandle(STD_OUTPUT_HANDLE);
	COORD pos;
	pos.X = x;
...
void Snake_EX::line(int n, char s)
{
	for (; n > 0; --n)
		cout << s;
}
void Snake_EX::Level(int n)
{
	FILE *f;
	int c = 0;// Хранение текущего считанного символа
	char *path = new char[MAX_PATH];
	if (n == 0)// Режим на выживание
	{
	...
	else// Режим карьеры
	{
		...
		_itoa(n, numlev, 10); // Функция itoa преобразует целое число value в строку string. К цифрам числа value подбираются ANSI символы типа char и записываются в строку string.
		strcat(path, numlev);// добавляет одну строку к другой
		strcat(path, ".txt");
		f = fopen(path, "a+");
		delete[]numlev;
	}
	SetCur(0, 7);// Отрисовка игрового поля
	setColor(YELLOW, BLACK);
	...
	for (int i = 0; i < height; ++i)// Обнуление массива
	{
	...
	bool error = 0;
	for (int i = 0; i < height; ++i)// Считывание с файла
	{
		for (int j = 0; j < width; ++j)
		{
			if (feof(f))// Преждевременный конец файла
			{
				...
				f = fopen(path, "w+");// Открытие и перезапись файла
				for (int i = 0; i < height; ++i)// Обнуление массива
				{
					for (int j = 0; j < width; ++j)
					{
				...
				for (int i = 0; i < height; ++i)// Заполнение файла нулями
				{
					for (int j = 0; j < width; ++j)
					{
						fputc('0', f);//Функция fputc записывает символ в поток и перемещает позицию индикатора положения.
					}
					if (i == height)
						break;
					fputc('\n', f);
			...
			c = fgetc(f);//Функция fgetc возвращает символ на который ссылается внутренний индикатор позиции файла указанного потока
	...
	setColor(YELLOW, BLACK);
	for (int y = 8; y < height + 8; ++y)// Отрисовка уровня
	{
...
void Snake_EX::editorLvl()
{
	...
	FILE *f;
	char *path = new char[MAX_PATH];// Путь к файлу
	strcpy(path, "levels/mylevel.txt");
	f = fopen(path, "ab+");
	...
	for (int i = 0; i < height; i++)// Считывание с файла
	{
		for (int j = 0; j < width; j++)
		{
			...
			{
				fclose(f);// Закрытие файла
				f = fopen(path, "wb+");// Открытие и перезапись файла
				for (int i = 0; i < height; i++)// Обнуление массива
				{
					for (int j = 0; j < width; j++)
					{
			...
			s = fgetc(f);
			if (s != 13 && s != 10)// Фильтр символа переноса строки и перевода каретки
			{
	...
	SetConsoleMode(h2, ENABLE_MOUSE_INPUT);// Включить режим использования меню
...
void Snake_EX::Gameprocces(bool career)
{
	system("cls");
	srand(time(0));
	Loading();
	...
	int pointsTemp = 0;
	bool flag = 1;// Состояние игры, 1 активно, 0 проигрыш
	int level = 1;// Текущий уровень
	int appleslost = 1;// Остаток яблок на уровне
	int livessnake = 5;// Количество жизней
	int dizCount = 2;// Количество дизориенататоров
	bool dizflag = 0;// Статус режима дизориенататора
	int Super_Speed_Count = 2;// Статус режима суперскорости
	bool SuperSpeedflag = 0;// Количество суперскоростей
	int delete2_Count = 2;//Количество сокращений
	bool delete2flag = 0;//Статус сокращений
	...
	if (career)
	{
		showLevelNumber(1);//ЗАСТАВКА уровня
	}

	SetCur(27, 2);// Ожидание пользователя
	setColor(CYAN, BLACK);
	cout << "For starting press Probel";
	while (_getch() != 32);
	SetCur(22, 2);// Стирание строки
	line(50, ' ');
	...
	while (true)// Игровой цикл
	{
		...
		SetCur(77, 5);// Текущий счет игрока
		setColor(YELLOW, BLACK);
		cout << pointsTemp << "    ";
		if (career)
		{
			setColor(PURPLE, BLACK);
			SetCur(61, 5);// Оставшееся количество жизней
			cout << livessnake;
			SetCur(33, 5);// Остаток яблок
			setColor(RED, BLACK);
			cout << appleslost << " ";
			setColor(CYAN, BLACK);// Уровень
			SetCur(12, 5);
			cout << level;
		}
		...
		if (!dizflag)// Нормальное движение
		{
		...
		if (career && appleslost <= 0)// Загрузка нового уровня
		{
			++level;
			if (level == 10)// Выйграшь
			{
				showLevelNumber(10);
				system("cls");
				return;
			...
			if (career)
			{
				...
				for (int i = 0; i < Super_Speed_Count; ++i)// Удаление старых суперскоростей
					field[superSpeed[i].Y - 8][superSpeed[i].X - 1] = 0;
				addSuperSpeed(++Super_Speed_Count);// Добавление новых суперскоростей
				for (int i = 0; i < delete2_Count; ++i)// Удаление старых суперскоростей
					field[delete2[i].Y - 8][delete2[i].X - 1] = 0;
				ADDDELETE2(++delete2_Count);// Добавление новых суперскоростей
			}
			continue;
		}
		for (int i = 0; career && (i < dizCount); ++i)// Попадание на дизориентат
		{
			if (snake[0].X == diz[i].X && snake[0].Y == diz[i].Y && field[diz[i].Y - 8][diz[i].X - 1] == 4)
			{
				...
				dizflag = 1;// Включение режима дизориентатора
				break;
			}
		}
		...
		for (int i = 0; career && (i < dizCount); ++i)// Попадание на суперскорость
		{
			if (snake[0].X == superSpeed[i].X && snake[0].Y == superSpeed[i].Y && field[superSpeed[i].Y - 8][superSpeed[i].X - 1] == 6)
			{
				...
				SuperSpeedflag = 1;// Включение режима суперскорости
				break;
			}
		}
		for (int i = 0; career && (i < dizCount); ++i)// Попадание на суперскорость
		{
			...
			{
				field[delete2[i].Y - 8][delete2[i].X - 1] == 0;// Удаление суперскорости
				delete2flag = 1;// Включение режима суперскорости
				break;
			}
		}
		...
		if (field[snake[0].Y - 8][snake[0].X - 1] == 1 || snake[0].Y <= 7 || snake[0].Y >= height + 8 || snake[0].X <= 0 || snake[0].X >= width + 1 || flag == 0)// Проверка на столкновение с препятствием или полем
		{
			--livessnake;// Потеря жизни
			pointsTemp = 0;
			if (livessnake && career)// Пройти уровень заново
			{
			...
			while (_getch() != 32);// Ожидание нажатия Probel
			system("cls");
			delete[]snake;
			return;
		}
...
void Snake_EX::addSuperSpeed(int k)
{
	delete[]superSpeed;
...
void Snake_EX::showLevelNumber(int n)
{
	SetConsoleTextAttribute(h, 14);
	for (int y = 8; y < height + 8; y++)// Очистка поля
	{
		for (int x = 1; x < width + 1; x++)
		{
			SetCur(x, y);
	...
	for (int i = 7; i < height + 8; ++i)// Отрисовка левой границы поля
	{
		SetCur(0, i);
		cout << char(219);
	}
}
void Snake_EX::ADDDELETE2(int k)
{
	delete[]delete2;
	delete2 = new COORD[k];
	for (int i = 0; i < k; i++)
