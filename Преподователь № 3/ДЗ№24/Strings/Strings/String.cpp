...
String::String(const uint capacity)
{
	if (capacity > 32 )
	{
		throw OutOfRangeException(capacity);
	}
	m_capacity = capacity;
...
String::String(const char character, const uint count)
{
	if (count > 30)
	{
		throw OutOfRangeException(character,count);
	}
	m_length = count;
	m_capacity = count + 1;
	m_string = new char[m_capacity];
...
String::String(const char* const string, const uint length)
{
	if ( length > 30)
	{
		throw OutOfRangeException(length);
	}
	m_length = length;
...
{
	if (length > 30 || startIndex > 30)
	{
		throw OutOfRangeException(startIndex, length);
	}
	m_length = length;
	m_capacity = m_length + 1;
	m_string = new char[m_capacity];
...
{
	if (thisStartIndex > 30 || otherStartIndex > 30 || length > 30)
	{
		throw OutOfRangeException(thisStartIndex,otherStartIndex,length);
	}
	if (ignoreCase == true)
	{
...
int String::IndexOf(const char character, const uint startIndex, const uint length) const
{
	if (startIndex > 30 || length > 30)
	{
		throw OutOfRangeException(character,startIndex,length);
	}
...
void String::Insert(const uint thisStartIndex, const String& string, const uint otherStartIndex)
{
	return Insert(0, string, 0);
}
void String::Insert(const uint thisStartIndex, const String& string, const uint otherStartIndex, const uint length)
{
	if (thisStartIndex > 30 || otherStartIndex > 30 || length > 30)
	{
		throw OutOfRangeException(thisStartIndex,otherStartIndex,length);
...
uint String::PadLeft(const uint totalWidth, const char padCharacter)
{
	if (totalWidth > 30)
	{
		throw OutOfRangeException(totalWidth,padCharacter);
	}
...
uint String::PadRight(const uint totalWidth) // дописать слева кол-во пробелов, чтобы в общем было переданное число. возвращает сколько дописали
{
	return PadLeft(totalWidth, ' ');
}

uint String::PadRight(const uint totalWidth, const char padCharacter)
{
	if (totalWidth > 30)
	{
		throw OutOfRangeException(totalWidth,padCharacter);
...
{
	if (trimCharacter < 0 || trimCharacter > 256)
	{
		throw OutOfRangeException(trimCharacter);
	}
