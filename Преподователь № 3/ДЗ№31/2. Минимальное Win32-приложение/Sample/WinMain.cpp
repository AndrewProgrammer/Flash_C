...
)
{
    ...
    WNDCLASSEX windowClass = {};//Обнуление всех полей
	...
	windowClass.hbrBackground = static_cast<HBRUSH>(GetStockObject(LTGRAY_BRUSH)); // Заполнение ярко-серым цветом.
    ...
    if (RegisterClassEx(&windowClass))
    {
        ...
        while (GetMessage(&message, window, 0U, 0U))
        {
